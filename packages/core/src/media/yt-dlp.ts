...
import type { MediaInfo, MediaFormat, SubtitleTrack } from '../../../shared/src/index.js';
import { runSpawn } from '../process/run-spawn.js';
...
async function runExtractOnce(url: string, extractorArgs: string): Promise<MediaInfo> {
  ...
  try {
    ...
    // yt-dlp exits with non-zero on some warnings, so check stderr.
    if (err.stdout) {
      try {
